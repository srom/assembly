...
#PBS -e error_concatenate_pfam_db_proka_r220_qsub.txt
#PBS -o output_concatenate_pfam_db_proka_r220_qsub.txt
...
if [ -z "${OUTPUT}" ]; then
    OUTPUT="${DB_PROKA_BASE}/db_proka_r220_genus_10s_Pfam-A_hits.csv"
fi
...
python -m src.postprocessing.concatenate_hmm_output \
        ...
        --cpu 32 \
        > ${HOME}/output_concatenate_pfam_db_proka_r220.txt \
        2> ${HOME}/error_concatenate_pfam_db_proka_r220.txt
